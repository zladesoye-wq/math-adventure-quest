...
const getLevelTitle = (worldKey, levelNum) => {
    const themes = {
...
const generateLevels = (worldId, worldKey) => {
    const levels = [];
    for (let i = 1; i <= 20; i++) {
        let difficultyModifier = 1.0;
        if (i <= 5)
            difficultyModifier = 0.5 + (i - 1) * 0.125;
        else if (i <= 10)
            difficultyModifier = 1.0 + (i - 6) * 0.125;
        else if (i <= 15)
            difficultyModifier = 1.5 + (i - 11) * 0.125;
        else
            difficultyModifier = 2.0 + (i - 16) * 0.25;
        levels.push({
            world_id: worldId,
            level_number: i,
    ...
    for (let i = 1; i <= 30; i++) {
        ...
        if (options.length === 0) {
            ...
            while (set.size < 4) {
                ...
                if (offset !== 0)
                    set.add((correctVal + offset).toString());
            }
    ...
    try {
        ...
        for (const world of worlds) {
            ...
            for (const question of questions) {
                await client.query('INSERT INTO questions (world_id, difficulty, skill_tag, question_text, options, correct_answer, hints) VALUES ($1, $2, $3, $4, $5, $6, $7)', [question.world_id, question.difficulty, question.skill_tag, question.question_text, question.options, question.correct_answer, question.hints]);
            }
        }
